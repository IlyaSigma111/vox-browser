...
export default function NewTabPage() {
  ...
  return (
    <div className="ntp">
      {settings.ntpShowClock && (
        <div className="ntp-clock">
          {time.getHours().toString().padStart(2, '0')}:{time.getMinutes().toString().padStart(2, '0')}
        </div>
      )}
      {settings.ntpShowDate && (
        <div className="ntp-date">
          {time.toLocaleDateString(settings.language === 'ru' ? 'ru-RU' : 'en-US', { weekday: 'long', day: 'numeric', month: 'long' })}
        </div>
      )}
      {settings.ntpShowSearch && (
